...
api.interceptors.request.use(
  (config) => {
    if (typeof window !== "undefined") {
      const token = localStorage.getItem("ergon_access_token");
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (typeof window !== "undefined") {
        ...
        try {
          ...
          localStorage.setItem("ergon_access_token", access_token);
          localStorage.setItem("ergon_refresh_token", new_refresh);
